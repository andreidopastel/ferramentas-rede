...
> rota.txt
# Process substitution (não pipe) para evitar subshell e permitir break/pkill funcionarem de verdade
...
done < <(tracepath -n "$TARGET" 2>/dev/null)

# --- [2] TESTE DE REDE LOCAL (DETECÇÃO DINÂMICA) ---
echo -e "\n${A}[2] TESTE DE REDE LOCAL${NC}"

# Tentativa 1: pega o salto 1 do arquivo rota.txt
GW_DETECTADO=$(grep -m 1 "^ *1:" rota.txt | grep -oE "([0-9]{1,3}\.){3}[0-9]{1,3}" | head -n 1)

# Tentativa 2: descobre o IP do próprio celular e assume final .1
...
# Tentativa 3: usa a tabela de vizinhos
if [[ -z "$GW_DETECTADO" ]]; then
    GW_DETECTADO=$(ip neigh show 2>/dev/null | grep "router" | awk '{print $1}' | head -n 1)
fi
...
if [[ -n "$GW_DETECTADO" ]]; then
    ...
    echo -e "${VM}Não foi possível detectar o roteador local.${NC}"
fi
...
echo -e "Latência Média ao Alvo: ${V}${LAT_AVG:-0} ms${NC}"
...
if command -v speedtest-cli &> /dev/null; then
    ...
    echo -e "${VM}Speedtest-cli não instalado. Instale com: pip install speedtest-cli${NC}"
fi
...
if [[ -n "$WIFI_JSON" && "$WIFI_JSON" != "{}" ]]; then
    ...
    RSSI=$(echo "$WIFI_JSON" | grep -oP '(?<="rssi": )-?[0-9]*' | head -n 1)

    echo -e "${AZ}SSID:${NC} ${SSID:-Desconhecido}"

    if [[ -n "$FREQ" ]]; then
        # Fórmula universal de canal (funciona para toda a faixa 2.4GHz e 5GHz)
    ...
    echo -e "${VM}Erro: API não respondeu. Ligue o GPS/Localização.${NC}"
fi
...
if [[ -z "$SCAN" || "$SCAN" == "[]" ]]; then
    ...
    echo "- GPS/Localização desligado"
    echo "- Permissão de localização negada ao Termux:API"
    echo "- Android ainda não atualizou o scan"
    echo -e "${A}Dica:${NC} rode manualmente: termux-wifi-scaninfo"
else
    echo -e "${AZ}Redes encontradas:${NC}\n"

    # =========================
    # LISTAGEM + ANÁLISE (mesma passada, sem subshell)
    # =========================
    declare -A canais24
    declare -A canais5
    ...
    while read -r rede; do
        ...
        # peso: quanto mais forte o sinal (RSSI mais próximo de 0), maior a interferência
        peso=$((100 + RSSI))
        [[ $peso -lt 0 ]] && peso=0
    ...
    # =========================
    # ESCOLHA FINAL (menor peso = menos congestionado)
    # =========================
    melhor24=1
    menor24=99999
    for c in 1 6 11; do
